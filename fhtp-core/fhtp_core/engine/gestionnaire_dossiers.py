...
from fhtp_core.models.dossier import AlerteRecours, Dossier
...
def _regime_pour_alerte(circuit: CircuitRemboursement) -> str:
    """Traduit le circuit de remboursement vers l'un des trois regimes
    attendus par AlerteRecours (section 6, champ `regime`) -- AMU/CAT/MIXTE.
    Cf. section 2.2 : l'alerte doit tenir compte du regime concerne."""
    if circuit == CircuitRemboursement.AMU_PLUS_PRIVE:
        return "MIXTE"
    if circuit == CircuitRemboursement.PRIVE_SEUL:
        return "CAT"
    return "AMU"  # AMU_SEUL, et par defaut pour DIRECT (pas de payeur a proprement parler)
...
class GestionnaireDossiers:
    ...
    def soumettre(
        ...
        if resultat_decision == StatutDossier.EN_VALIDATION_LOCALE:
            ...
            if resultat_decision == DecisionFinale.AUDIT_APPROFONDI:
                # Cf. R-TG-009 (section 2.2) : tout rejet/anomalie doit
                # declencher une alerte recours contextualisee, jamais se
                # limiter au seul motif technique. Ne fige pas de delai
                # unique (section 2.2 : "cette alerte ne fige pas un delai
                # unique") -- laisse ce champ indicatif, jamais garanti.
                dossier = dossier.model_copy(
                    update={
                        "alerte_recours": AlerteRecours(
                            active=True,
                            regime=_regime_pour_alerte(dossier.circuit_remboursement),
                            delai_indicatif=(
                                "Delai contextualise selon le regime et le motif -- "
                                "non fige (section 2.2)"
                            ),
                            action_recommandee=(
                                "Examiner les motifs listes et regulariser le dossier, "
                                "ou engager la voie de recours applicable au regime"
                            ),
                        )
                    }
                )
