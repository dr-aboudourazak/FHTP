...
class GestionnaireDossiers:
    ...
    def _enrichir_dossier(self, dossier: Dossier) -> Dossier:
        ...
        Le type de prescripteur (paramedical ou non) et le type de
        formation (clinique privee ou non) ne sont recalcules QUE si le
        registre correspondant est fourni ET que la reference existe --
        sinon, la valeur deja presente sur le Dossier (champ precalcule
        fourni par l'appelant, potentiellement None) est conservee telle
        quelle. Ca permet a l'ancien mode de fonctionnement (sans registre)
        de continuer a fonctionner a l'identique.
        """
        ...
        mise_a_jour_dossier: dict = {"medicaments": nouveaux_medicaments}
